...
status              VARCHAR(20)  NOT NULL   -- running|passed|failed|pending_approval
target_environment  VARCHAR(60)  NOT NULL
checks_performed    JSON         NULL       -- row counts, trial balance, spot checks
duration_seconds    INT
error               TEXT         NULL
requested_by        BIGINT       NULL FK    -- BR-CORE-13-009 dual authorisation, added during implementation
approved_by         BIGINT       NULL FK    -- must differ from requested_by for target_environment = 'production'
tested_at           TIMESTAMP    NULL
```

> **Implementation note (added during CORE-13 build):** the spec is silent on how BR-CORE-13-009's "restores in production require dual authorisation" is data-modelled. Rather than a new table, a production restore is recorded as one `restore_tests` row with `target_environment = 'production'`, `status` starting at `pending_approval`, and `requested_by`/`approved_by` columns added above — the same requester-then-different-approver shape `period_reopen_requests` (CORE-03 §4) already uses. `tested_at` is nullable to allow a row to exist before the restore actually runs.
