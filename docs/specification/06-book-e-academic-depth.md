...
> **Implementation note (added when ACA-06 was built).** The spec does not say how the continuous weight interacts with `ACA-01.subjects.coursework_weight_percent`'s existing coursework/examination split. `ComputeTermSubjectResultsAction` (ACA-05) takes the active `AssessmentInstrument.default_weight_percent` as the continuous share and applies `coursework_weight_percent` to split what remains between coursework and examination — e.g. a 30% SBP weight with a 60% coursework split yields 30% SBP / 42% coursework / 28% examination. Only a *verified* outcome is used (BR-ACA-06-014); a merely marked or moderated project is treated as if no continuous outcome existed yet. Revisit if a future circular defines the three-way split explicitly.
