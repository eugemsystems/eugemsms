...
created_at
  ...
  -- DB grants: INSERT, SELECT. No UPDATE except the three narrow columns below.
  -- Permitted UPDATE columns: status, reversed_by_journal_id, approved_by  (enforced by trigger)
  -- (approved_by added during implementation: a draft manual journal has no
  --  approver until ACT-ApproveManualJournal runs, §5/BR-FIN-01-018 — the
  --  original two-column list left no path for that column to ever be set.)
