...
/**
 * spatie/laravel-permission's own migration, copied in rather than
 * published so it lives with the rest of Book A's schema. `config/
 * permission.php` turns on the "teams" feature with `team_foreign_key`
 * renamed to `school_id` — this is what gives `roles`/`model_has_roles`/
 * `model_has_permissions` the `school_id` column the spec's schema
 * requires (Book A CORE-05 §2: "NULL = system template"; roles are
 * assigned per school, BR-CORE-05-012).
 */
